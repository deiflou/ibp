...
extern "C" ANITOOLS_EXPORT QHash<QString, QString> getAnitoolsPluginInfo()
{
    ...
    info.insert("id", "anitools.imagefilter.lowpassiihc");
    info.insert("version", "0.1.0");
    info.insert("name", QObject::tr("Low Pass IIH Correction"));
    info.insert("description", QObject::tr("Removes the artifacts due to a bad illumination using a low pass filtering approach"));
    info.insert("tags", QObject::tr("Illumination, Smooth"));
