...
extern "C" ANITOOLS_EXPORT QHash<QString, QString> getAnitoolsPluginInfo()
{
    ...
    info.insert("id", "anitools.imagefilter.surfacefittingiihc");
    info.insert("version", "0.1.0");
    info.insert("name", QObject::tr("Surface Fitting IIH Correction"));
    info.insert("description", QObject::tr("Removes the artifacts due to a bad illumination using a surface fitting approach"));
